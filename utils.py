...
def calc_prf_iou(y_pred, y_true, threshold=0.5, eps=1e-7):
    """Binary segmentation metrics: Precision / Recall / F1 / IoU."""
    with torch.no_grad():
        assert y_pred.shape == y_true.shape

        pred = (y_pred >= threshold).float()
        gt = (y_true >= threshold).float()
    ...
    return np.float64(precision), np.float64(recall), np.float64(f1), np.float64(iou)
