...
class ValDataset(Dataset):
    def __init__(self, dataset, inp_size=None, augment=False, return_meta=False):
    ...
    def __getitem__(self, idx):
        ...
        if self.return_meta:
            if not hasattr(self.dataset, 'get_meta'):
                raise AttributeError('Dataset does not provide get_meta() required for visualization export.')
            sample.update(self.dataset.get_meta(idx))
        return sample
